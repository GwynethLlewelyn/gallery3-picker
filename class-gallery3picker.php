...
/* Admin page includes */
add_filter( 'media_upload_tabs', [ 'gallery3Picker', 'tab_gallery' ] );
add_filter( 'media_upload_gallery3_picker', [
	'gallery3Picker',
	'tab_select_gallery',
]);

add_filter('media_upload_type_gallery3_form', [
	'gallery3Picker',
	'media_upload_type_gallery3_form',
]);
add_action('admin_menu', ['gallery3Picker', 'add_gallery3_picker_menu']);

/* Javascript support */
add_action('wp_ajax_gallery3proxy', ['gallery3Proxy', 'requestHandler']);

class gallery3Picker
{
	function add_gallery3_picker_menu() {
		add_options_page(
			__('Gallery3 settings'),
			__('Gallery3 media picker'),
			8,
			basename(__FILE__),
			['gallery3Picker', 'gallery3_picker_options_page']
		);
	}
	...
	function tab_select_gallery()
	{
		...
		wp_enqueue_script(
			'jquery.jstree.strip.min',
			$plugin_url . '/jquery.jstree.min.js'
		);
		wp_enqueue_script('gallery3_picker', $plugin_url . '/gallery.js');

		if (
			$options['gallery3_url'] == '' ||
			$options['gallery3_status'] == 'failed'
		) {
			return wp_iframe([
				'gallery3Picker',
				'media_upload_type_gallery3_config_error',
			]);
		} elseif ($_GET['gallery3_picker_id'] && $_GET['deleted'] != 1) {
			if ($_GET['gallery3_import'] == 'true') {
				return wp_iframe(
					[
						'gallery3Picker',
						'media_upload_type_gallery3_native_insert_form',
					],
					'gallery3',
					$errors,
					$id
				...
				return wp_iframe(
					[
						'gallery3Picker',
						'media_upload_type_gallery3_insert_form',
					],
					'gallery3',
					$errors,
					$id
			...
			return wp_iframe(
				['gallery3Picker', 'media_upload_type_gallery3_form'],
				'gallery3',
				$errors,
				$id
			);
	...
	function media_upload_type_gallery3_insert_form(
		$type = 'file',
		$errors = null,
		$id = null
	) {
		$node_id = isset( $_GET['gallery3_picker_id'] )
				 ? intval( $_GET['gallery3_picker_id'] )
				 : 0;
		$post_id = isset( $_REQUEST['post_id'] )
				 ? intval( $_REQUEST['post_id'] )
				 : 0;
		$back    = admin_url(
			"media-upload.php?tab=gallery3_picker&post_id=$post_id"
		);
		$thumb_url  = isset( $_REQUEST['thumb'] )
					? wp_unslash( $_REQUEST['thumb'] )
					: '';
			...
			<table>
				...
				<tr class="url pickerCanEmbed" style="display: none;">
					...
					<td class='field'>
						...
						<button type='button' id="urlButtonPost" class='button urlpost' onclick="buttonToField(this,'pickerUrl');" title="$perma_link">Post URL</button>
...
EOT;
	...
	function media_upload_type_gallery3_native_insert_form(
		$type = 'file',
		$errors = null,
		$id = null
	) {
		...
		$form_action_url = admin_url(
			"media-upload.php?type=file&tab=type&post_id=$post_id"
		);
		...
		media_upload_header();
		?>
		<form enctype="multipart/form-data" method="post" action="<?php echo esc_attr(
  	$form_action_url
  ); ?>" class="media-upload-form type-form validate" id="<?=wp_slash( $type ) ; ?>-form">
		...
		<p>
		<input type="submit" class="button savebutton hide-if-no-js" name="save" value="<?php esc_attr_e(
  	"Save all changes"
  ); ?>" />
		</p>
		</form>
	...
	function media_upload_type_gallery3_form($type = 'file', $errors = null, $id = null) {
		...
		<form enctype="multipart/form-data" method="post" action="<?php echo esc_attr(
  	$form_action_url
  ); ?>" class="media-upload-form type-form validate" id="<?php echo $type; ?>-form">
		<input type="submit" class="hidden" name="save" value="" />
		<input type="hidden" name="post_id" id="post_id" value="<?php echo (int) $post_id; ?>" />
		<?php wp_nonce_field("media-form"); ?>

		<h3 class="media-title"><?php
  _e("Add media files from ");
  echo $options["gallery3_name"];
  ?></h3>
  	...
  	basename(__FILE__); ?>&updated=true">
		...
		<table width="700px" cellspacing="2" cellpadding="5" class="editform">
			<tr valign="center">
				...
				<td><input type="text" name="gallery3_config_url" id="gallery3_config_url_inp" value="<?php echo $optionarray_def[
    	"gallery3_config_url"
    ]; ?>" size="50" /></td>
			</tr>
			<?php if (
   	...
   	$optionarray_def["gallery3_url"] !=
   		$optionarray_def["gallery3_config_url"]
   ): ?>
			<tr valign="center">
				<td width="300px" scope="row"><?php _e("Gallery3 REST API"); ?></td>
				<td><input type="text" disabled="true" name="gallery3_ro_url" id="gallery3_url_inp" value="<?php echo $optionarray_def[
    	"gallery3_url"
    ]; ?>" size="50" /></td>
			</tr>
			<?php endif; ?>
			<tr valign="center">
				...
				<td><input type="text" name="gallery3_name" id="gallery3_name_inp" value="<?php echo $optionarray_def[
    	"gallery3_name"
    ]; ?>" size="50" /></td>
			</tr>
			<tr valign="center">
				<td width="300px" scope="row"><?php _e("API key (if any)"); ?></td>
				<td><input type="text" name="gallery3_api_key" id="gallery3_api_key_inp" value="<?php echo $optionarray_def[
    	"gallery3_api_key"
    ]; ?>" size="50" /></td>
			</tr>
		</table>
			<div class="submit">
				...
				<?php if ($optionarray_def["gallery3_query_type"] == "3.0.0"): ?>
					...
					<?php endif; ?>
					<span>Maximum acceptable request var length: <?= $optionarray_def[
     	"gallery3_maxlength"
     ] ?></span><br />
				<?php else: ?>
					<p>
